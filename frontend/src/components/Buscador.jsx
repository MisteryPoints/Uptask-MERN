...
const Busqueda = () => {
    ...
    const proyectosFiltrados = busqueda === '' ? [] : proyectos.filter( proyecto => proyecto.nombre.toLowerCase().includes( busqueda.toLowerCase() ) || proyecto.cliente.toLowerCase().includes( busqueda.toLowerCase() ) )
        ...
        <Transition.Root show={ buscador } as={Fragment} afterLeave={ () => setBusqueda('') }>
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={ handleBuscador }>
                ...
                <Transition.Child
                    ...
                    enterFrom="opacity-0 scale-50 translate-x-[25%] translate-y-[-200%]"
                    enterTo="opacity-100 scale-100 translate-x-0"
                    leave="transform transition ease-in duration-400"
                    leaveFrom="opacity-100 scale-100 translate-x-0"
                    leaveTo="opacity-0 scale-50 translate-x-[25%] translate-y-[-200%]"
                >
                <Combobox
                    as="div"
                ...
                >
                    <div className="relative">
                        <Combobox.Input
                            className="h-12 w-full border-0 bg-transparent pl-4 pr-4 text-gray-800 placeholder-gray-400 focus:ring-0 sm:text-sm focus:outline-none"
                            placeholder="Buscar..."
